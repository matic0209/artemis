...
//!
//! This module provides constraint-based optimization for MEV arbitrage execution strategies
//! using the Z3 SMT solver. This is a STRATEGY-LEVEL optimizer, meaning it optimizes
//! execution parameters and strategy selection, not general execution plans.
//!
//! ## Architecture Position
//!
//! ```text
//! Detectors (find opportunities)
//!     ↓
//! Strategy Optimizers (THIS MODULE - optimize strategy parameters using Z3)
//!     ↓
//! Validators (validate feasibility)
//!     ↓
...
//!
//! ## Difference from general Optimizer trait
//!
//! - **General Optimizer** (in optimizers/): Optimizes execution plans (gas, ordering, etc.)
//! - **Strategy Optimizer** (THIS): Optimizes MEV strategy parameters (amounts, paths, etc.)
...
/// Z3-based strategy optimizer
///
/// Uses constraint solving to find optimal parameters for arbitrage strategy execution.
/// This is different from general execution plan optimizers.
pub struct Z3StrategyOptimizer {
    /// Z3 configuration
...
pub type Z3ArbitrageOptimizer = Z3StrategyOptimizer;
...
impl Z3StrategyOptimizer {
    /// Create new Z3 strategy optimizer
...
#[async_trait]
impl Optimizer for Z3StrategyOptimizer {
    async fn optimize(
        &mut self,
        plan: &ExecutionPlan,
...
mod tests {
    ...
    fn test_z3_optimizer_creation() {
        ...
        let optimizer = Z3StrategyOptimizer::new(config);
