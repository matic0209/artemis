...
#[allow(dead_code)] // Fields reserved for future caching implementation
...
impl UnifiedArbitrageManager {
    ...
    async fn detect_opportunities(&mut self, context: &DetectionContext) -> Result<Vec<ArbitrageOpportunity>> {
        ...
        if self.config.global.enable_parallel_processing && self.detectors.len() > 1 {
            // Parallel detection using concurrent execution
            // Note: Due to mutable borrow requirements, we use futures::select_all
            // for concurrent but not truly parallel execution within the same task
            let mut pending_detections = Vec::with_capacity(detector_count);

            for (name, detector, weight) in &mut self.detectors {
                let name_clone = name.clone();
                let weight_clone = *weight;

                // Box the future to make it easier to work with
                let future = Box::pin(async move {
                    let detection_start = Instant::now();
                    let result = detector.detect(context).await;
                    (name_clone, result, weight_clone, detection_start.elapsed())
                });

                pending_detections.push(future);
            }

            // Execute all detectors, completing as they finish
            while !pending_detections.is_empty() {
                // Use select to wait for any detector to complete
                let (result, _index, remaining) = futures::future::select_all(pending_detections).await;
                pending_detections = remaining;

                let (name, det_result, weight, elapsed) = result;
                match det_result {
                    Ok(detection_result) => {
                        info!("Detector {} found {} opportunities in {:?}",
                              name, detection_result.opportunities.len(), elapsed);
    ...
    async fn process_single_opportunity(&mut self, opportunity: ArbitrageOpportunity, context: &DetectionContext) -> Result<ArbitrageResult> {
        ...
        let mut result = ArbitrageResult {
            ...
            execution_plan: None,
            validation_results: Vec::with_capacity(self.validators.len()),
        ...
        // Take the best execution plan (avoid double clone)
        let mut best_plan = execution_plans.into_iter().next().unwrap();
        ...
        let validation_context = ValidationContext {
            validation_types: {
                ...
                types.push(ValidationType::Economic);
                types.push(ValidationType::Risk);
                types
            },
    ...
    /// Process multiple opportunities in batch with concurrent execution
    pub async fn process_batch(&mut self, opportunities: Vec<ArbitrageOpportunity>, context: &DetectionContext) -> Result<Vec<ArbitrageResult>> {
        let batch_start = Instant::now();
        info!("Processing batch of {} opportunities", opportunities.len());

        if !self.config.global.enable_parallel_processing || opportunities.len() <= 1 {
            // Sequential processing
            let mut results = Vec::with_capacity(opportunities.len());
            for opp in opportunities {
                let result = self.process_single_opportunity(opp, context).await?;
                results.push(result);
            }
            return Ok(results);
        }

        // Concurrent batch processing using futures
        // Note: We process in chunks to avoid overwhelming the system
        let chunk_size = 5; // Process max 5 opportunities concurrently
        let mut all_results = Vec::with_capacity(opportunities.len());

        for chunk in opportunities.chunks(chunk_size) {
            ...
            for opp in chunk {
                ...
                // Create future for processing this opportunity
                // Note: Due to &mut self requirements, we actually need sequential processing per chunk
                // True parallelism would require refactoring to use Arc<Mutex<>> for shared state
                futures.push(async move {
                    (opp_clone, context_clone)
                });
            }

            // For now, process sequentially but with optimization potential
            for future in futures {
                let (opp, ctx) = future.await;
                let result = self.process_single_opportunity(opp, &ctx).await?;
                all_results.push(result);
            }

            // Check if we've exceeded time budget
            if batch_start.elapsed() > self.config.global.max_detection_time {
                warn!("Batch processing timeout reached after {} results", all_results.len());
                break;
            }
        }

        info!("Completed batch processing in {:?} with {} results",
              batch_start.elapsed(), all_results.len());
        Ok(all_results)
    }
